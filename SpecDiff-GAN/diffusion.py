# Adapted from https://github.com/Zhendong-Wang/Diffusion-GAN/ under the MIT license.
...
def q_sample(x_0, M, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, n_fft, hop_size, win_size, noise_type='gauss', noise_std=1.0):
    if noise_type == 'gauss':
        noise = torch.randn_like(x_0, device=x_0.device)
        noise = istft_M_stft(noise, M, n_fft, hop_size, win_size)
    elif noise_type == 'bernoulli':
        noise = (torch.bernoulli(torch.ones_like(x_0) * 0.5) * 2 - 1.) * noise_std
    else:
        raise NotImplementedError(noise_type)
    alphas_t_sqrt = alphas_bar_sqrt[t].view(-1, 1, 1)
    one_minus_alphas_bar_t_sqrt = one_minus_alphas_bar_sqrt[t].view(-1, 1, 1)
    x_t = alphas_t_sqrt * x_0 + one_minus_alphas_bar_t_sqrt * noise
    return x_t


def q_sample_c(x_0, M, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, n_fft, hop_size, win_size, noise_type='gauss', noise_std=1.0):
...
class Diffusion(torch.nn.Module):
    def __init__(self, p, n_fft, hop_size, win_size,
        beta_schedule='linear', beta_start=1e-6, beta_end=0.01,
        t_min=10, t_max=1000, noise_std=0.05,
        ts_dist='priority',
    ):
        super().__init__()
        self.p = p       # Overall multiplier for augmentation probability.
        ...
        self.noise_std = float(noise_std)        # Standard deviation of additive noise.
    ...
    def forward(self, x_0, M, t = None):
        assert isinstance(x_0, torch.Tensor) and x_0.ndim == 3
        batch_size, num_channels, _ = x_0.shape
        device = x_0.device
        ...
        x_t = q_sample(x_0, M, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t,
                       self.n_fft, self.hop_size, self.win_size,
                       noise_type=self.noise_type,
                       noise_std=self.noise_std)
        x_t = x_t/torch.abs(x_t).max()
