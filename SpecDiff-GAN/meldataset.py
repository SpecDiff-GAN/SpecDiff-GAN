...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False, return_filter = True):
    if torch.min(y) < -1.:
...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            ...
            mel, M = mel_spectrogram(audio, self.n_fft, self.num_mels,
        ...
        mel_loss = mel_spectrogram(audio, self.n_fft, self.num_mels,
                                   ...
                                   center=False, return_filter=False)

        return (mel.squeeze(), audio.squeeze(0), filename, mel_loss.squeeze(), M.squeeze())
